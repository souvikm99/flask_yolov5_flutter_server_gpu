...
def detect(opt, model):
    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        # print(f'non NON MAX -> {pred}')
        # Apply NMS
        pred = non_max_suppression(pred, opt.conf_thres, opt.iou_thres, opt.classes, opt.agnostic_nms, max_det=opt.max_det)
        # print(f'NON MAX -> {pred}')
        dt[2] += time_sync() - t3
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)):
                        bboxes = output[0:4]
                        id = output[4]  # Assuming this is always numeric
                        cls = output[5]

                        c = int(cls)  # integer class
                        label = f'{id} {names[c]} {conf:.2f}'
                        label2 = f'{id} {names[c]}'  # Simplified label without confidence
                        annotator.box_label(bboxes, label, color=colors(c, True))


                        # Track unique IDs for each class
                        if names[c] not in unique_ids_per_class:
                            unique_ids_per_class[names[c]] = set()
                        unique_ids_per_class[names[c]].add(id)
            ...
            class_counts = {class_name: len(ids) for class_name, ids in unique_ids_per_class.items()}
            # print("Class counts:", class_counts)
                # Construct the new filename for the counts
            counts_file_path = os.path.join(str(Path(out)), txt_file_name + '_counts.txt')  
